...
export default function InlineValidator({ model, onModelCorrected, onGoPredict }: Props) {
  ...
  const generatePredictions = useCallback(async (m: CognitiveModel) => {
    ...
    try {
      const data = await postJSON<{ predictions: DimensionPrediction[] }>(
        "/api/validate",
        { model: m },
      );
  ...
  const handleUpdateModel = useCallback(async () => {
    ...
    try {
      const data = await postJSON<{ corrected_model: CognitiveModel; changes_summary: string }>(
        "/api/validate/update",
        { model, judgments: judgmentList, corrections: correctionList },
      );
      const updated = data.corrected_model;
