...
// 被试目录：NOUS_SUBJECT_DIR（绝对路径或相对仓库根），默认 data/samples（示例人物，无 signals_history 时返回空）
const SUBJECT_DIR = process.env.NOUS_SUBJECT_DIR
  ? (isAbsolute(process.env.NOUS_SUBJECT_DIR) ? process.env.NOUS_SUBJECT_DIR : join(process.cwd(), "..", process.env.NOUS_SUBJECT_DIR))
  : join(process.cwd(), "..", "data", "samples");
